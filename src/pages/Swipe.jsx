...
function GenderBadge({ gender }) {
  ...
  return (
    <div style={{ textAlign:'center' }}>
      <div style={{ display:'inline-flex', alignItems:'center', gap:5, padding:'4px 12px', borderRadius:20, background:cfg.bg, color:cfg.color, fontFamily:"'Inter',system-ui", fontSize:11, fontWeight:700, marginBottom:14 }}>
        {cfg.label}
      </div>
    </div>
  )
...
export default function Swipe({ session, coupleId, filters }) {
  ...
  const [history, setHistory]     = useState([]) // {idx, swipeId, direction}
  ...
  async function loadNames() {
    ...
    if (filters.style?.length) {
      ...
      filtered = filtered.filter(n => n.style_tags?.some(tag => styleLower.some(s => tag.includes(s.split('/')[0].trim()))))
    }
    if (filters.collections?.length) {
      const collTags = filters.collections.map(c => COLLECTION_TAG_MAP[c]).filter(Boolean)
      filtered = filtered.filter(n => collTags.some(tag => n.style_tags?.some(t => t.toLowerCase().includes(tag.toLowerCase()))))
    }
    if (filters.syllables?.length) {
      filtered = filtered.filter(n => filters.syllables.includes(String(n.syllables)))
    }
  ...
  async function registerSwipe(name, direction) {
    // Delete existing swipe if any (for undo-redo)
  ...
  function animateSwipe(dir) {
    ...
    setTimeout(() => {
      if (card) { card.style.transition = ''; card.style.transform = ''; card.style.opacity = '1' }
      setSwiping(false)
      setLikeOpacity(0); setNopeOpacity(0)
    }, 400)
  }
  ...
  async function swipeRight() {
    ...
    setHistory(h => [...h, { idx: index, swipeId, direction: 'like' }])
  ...
  async function swipeLeft() {
    ...
    setHistory(h => [...h, { idx: index, swipeId, direction: 'dislike' }])
    setTimeout(() => setIndex(i => i + 1), 390)
  }
  ...
  async function undo() {
    ...
    // Delete that swipe from DB
    if (last.swipeId) {
      await supabase.from('swipes').delete().eq('id', last.swipeId)
      // Remove match if it was a like
      if (last.direction === 'like') {
        await supabase.from('matches').delete()
          .eq('couple_id', coupleId).eq('name_id', names[last.idx].id)
      }
    }
    ...
    if (card) {
      ...
      setTimeout(() => { if(card) { card.style.transform='scale(1)'; setTimeout(()=>card.style.transition='',260) } }, 260)
    }
  }

  function onMouseDown(e) { isDragging.current=true; startX.current=e.clientX; startY.current=e.clientY; document.addEventListener('mousemove',onMouseMove); document.addEventListener('mouseup',onMouseUp) }
  function onMouseMove(e) { if(isDragging.current) moveDrag(e.clientX-startX.current,e.clientY-startY.current) }
  function onMouseUp(e)   { if(!isDragging.current) return; isDragging.current=false; document.removeEventListener('mousemove',onMouseMove); document.removeEventListener('mouseup',onMouseUp); endDrag(e.clientX-startX.current) }
  function onTouchStart(e){ isDragging.current=true; startX.current=e.touches[0].clientX; startY.current=e.touches[0].clientY; document.addEventListener('touchmove',onTouchMove,{passive:false}); document.addEventListener('touchend',onTouchEnd) }
  function onTouchMove(e) { if(!isDragging.current) return; e.preventDefault(); moveDrag(e.touches[0].clientX-startX.current,e.touches[0].clientY-startY.current) }
  function onTouchEnd(e)  { if(!isDragging.current) return; isDragging.current=false; document.removeEventListener('touchmove',onTouchMove); document.removeEventListener('touchend',onTouchEnd); endDrag(e.changedTouches[0].clientX-startX.current) }

  function moveDrag(dx,dy) {
    const card = cardRef.current
    if (!card || swiping) return
    card.style.transform = `translate(${dx}px,${dy}px) rotate(${dx*.07}deg)`
    if (dx>40)       { setLikeOpacity(Math.min(1,(dx-40)/80)); setNopeOpacity(0) }
    else if (dx<-40) { setNopeOpacity(Math.min(1,(-dx-40)/80)); setLikeOpacity(0) }
  ...
  function endDrag(dx) {
    ...
    if (dx>80) swipeRight()
      ...
      card.style.transition='transform .28s'; card.style.transform=''
      setLikeOpacity(0); setNopeOpacity(0)
  ...
  const nameFontSize = current ? (current.name.length > 10 ? 32 : current.name.length > 7 ? 42 : 54) : 54

  return (
    <>
      {match && <MatchOverlay name={match} onContinue={() => setMatch(null)} />}

      <div style={{ padding:'14px 22px 12px', display:'flex', alignItems:'center', justifyContent:'flex-end', background:'#fff', borderBottom:'1px solid #EDEBE9' }}>
      ...
      <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', padding:'20px 24px', position:'relative' }}>
        {remaining > 2 && <div style={{ position:'absolute', width:'calc(100% - 80px)', height:'72%', background:'#fff', borderRadius:26, border:'1px solid #EDEBE9', top:40, zIndex:0, opacity:.3, transform:'scale(.88)' }} />}
        {remaining > 1 && <div style={{ position:'absolute', width:'calc(100% - 56px)', height:'74%', background:'#fff', borderRadius:26, border:'1px solid #EDEBE9', top:30, zIndex:1, opacity:.6, transform:'scale(.94)' }} />}

        {current ? (
          <div ref={cardRef}
            style={{ width:'100%', maxWidth:320, background:'#fff', borderRadius:26, padding:'28px 24px 24px', textAlign:'center', border:'1.5px solid #C8C4C2', position:'relative', zIndex:2, cursor:'grab', userSelect:'none', boxShadow:'0 6px 28px rgba(26,14,14,.09)' }}
          ...
          >
            <div style={{ position:'absolute', top:20, right:16, padding:'5px 14px', borderRadius:10, fontFamily:"'Poppins',system-ui", fontSize:15, fontWeight:700, border:'2.5px solid #145030', color:'#145030', transform:'rotate(15deg)', opacity:likeOpacity, pointerEvents:'none' }}>LIKE</div>
            <div style={{ position:'absolute', top:20, left:16, padding:'5px 14px', borderRadius:10, fontFamily:"'Poppins',system-ui", fontSize:15, fontWeight:700, border:'2.5px solid #7C2020', color:'#7C2020', transform:'rotate(-15deg)', opacity:nopeOpacity, pointerEvents:'none' }}>NOPE</div>

            <GenderBadge gender={current.gender} />
            <div style={{ fontFamily:"'Poppins',system-ui", fontSize:nameFontSize, fontWeight:700, color:'#1A0E0E', lineHeight:1, marginBottom:8, letterSpacing: nameFontSize > 40 ? -1.5 : -0.5, wordBreak:'break-word' }}>
              {current.name}
            </div>
            ...
            <div style={{ fontFamily:"'Inter',system-ui", fontSize:13, color:'#5A4040', lineHeight:1.55, marginBottom:18, padding:'0 4px' }}>{current.meaning}</div>
      ...
      {current && (
        <div style={{ display:'flex', justifyContent:'center', alignItems:'center', gap:16, padding:'16px 24px 20px' }}>
          <button onClick={swipeLeft} style={{ width:64,height:64,borderRadius:'50%',border:'2.5px solid #E09090',cursor:'pointer',background:'#FDEAEA',display:'flex',alignItems:'center',justifyContent:'center',transition:'transform .15s' }} onMouseEnter={e=>e.currentTarget.style.transform='scale(1.09)'} onMouseLeave={e=>e.currentTarget.style.transform='scale(1)'}>
            <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#7C2020" strokeWidth="2.8" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
          </button>
          <button onClick={undo} style={{ width:48,height:48,borderRadius:'50%',border:'2px solid #C8C0C0',cursor:'pointer',background:'#ECEAEA',display:'flex',alignItems:'center',justifyContent:'center',transition:'transform .15s',opacity:history.length?1:0.3 }} onMouseEnter={e=>e.currentTarget.style.transform='scale(1.09)'} onMouseLeave={e=>e.currentTarget.style.transform='scale(1)'}>
          ...
          <button onClick={swipeRight} style={{ width:64,height:64,borderRadius:'50%',border:'2.5px solid #165C2E',cursor:'pointer',background:'#2E9954',display:'flex',alignItems:'center',justifyContent:'center',transition:'transform .15s' }} onMouseEnter={e=>e.currentTarget.style.transform='scale(1.09)'} onMouseLeave={e=>e.currentTarget.style.transform='scale(1)'}>
